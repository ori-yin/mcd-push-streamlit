...
def parse_csv(file_or_path):
    ...
    for _, row in df.iterrows():
        ...
        pt  = str(row.get(g('ptype', ''), '')).strip() or '未知'
...
def calc_s4_data(owner_agg, DATE_Y, DATE_P, DATE_W, ptypes_seen=None):
    ...
    result = {}
    for ptype in sorted(ptypes_seen):
        owners = set()
...
with col2:
    ...
    if not uploaded:
        ...
        try:
            ...
            # ── S3-a: 计划类型触达对比（动态）──────────────────────
            ptypes_sorted = sorted(ptypes_seen)
            aarr_pt = next((p for p in ptypes_sorted if 'aarr' in p.lower()), ptypes_sorted[0] if ptypes_sorted else '')
            other_pts = [p for p in ptypes_sorted if p != aarr_pt]
            # S3-a traces: one line per plan type, all channels aggregated
            PTYPE_COLORS = ['#DA291C', '#46B5D8', '#FFC72C', '#00A04A', '#888888']
            s3a_traces = []
            if aarr_pt:
                s3a_traces.append({
                    'name': aarr_pt,
                    'color': '#DA291C',
                    'y': [sum(rows_raw.get(d,{}).get(ch,{}).get(aarr_pt,{}).get('reach',0) for ch in ch_list) for d in all_dates]
                })
            for idx, pt in enumerate(other_pts):
                s3a_traces.append({
                    'name': pt,
                    'color': PTYPE_COLORS[(idx+1) % len(PTYPE_COLORS)],
                    'y': [sum(rows_raw.get(d,{}).get(ch,{}).get(pt,{}).get('reach',0) for ch in ch_list) for d in all_dates]
                })
            # 兼容旧JS变量（单aarr + 单other）
            reach_aarr_js   = json.dumps(s3a_traces[0]['y'] if s3a_traces else [], ensure_ascii=False)
            reach_other_js  = json.dumps([sum(rows_raw.get(d,{}).get(ch,{}).get(p,{}).get('reach',0) for ch in ch_list for p in other_pts) for d in all_dates], ensure_ascii=False) if other_pts else '[]'
            s3a_traces_js  = json.dumps(s3a_traces, ensure_ascii=False)
            ...
            ptype_label = ' / '.join(sorted(ptypes_seen))
            html = f"""<!DOCTYPE html>
...
<div class="card">
  ...
  <div class="card-title">3. 渠道 × 计划类型（{ptype_label}）</div>
...
(function() {{
  ...
  const traces = Object.keys(rawData).map(ch => ({{
    ...
    y: rawData[ch].map(d => Object.values(d).reduce((s, v) => s + (v.reach||0), 0)),
...
(function() {{
  ...
  const s3aTraces = {s3a_traces_js};

  const s3aTraceObjects = s3aTraces.map(t => {{

    return {{ x: {x_dates_js}, y: t.y, type: 'scatter', mode: 'lines+markers', name: t.name,

      line: {{ color: t.color, width: 2 }}, marker: {{ size: 5 }}, hovertemplate: '%{{y:.0f}}<extra></extra>' }};

  }});

  Plotly.newPlot('chart-s3a', s3aTraceObjects, {{

    title: {{ text: '触达成功趋势', font: {{ size: 13, color: '#DA291C' }}, x: 0.5 }},
