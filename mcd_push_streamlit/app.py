...
with col2:
    if not uploaded:
        ...
        try:
            ...
            reach_ptype_js = {}
            for pt_i, pt in enumerate(ptype_list):
                reach_ptype_js[pt] = json.dumps([sum(rows_raw.get(d,{}).get(ch,{}).get(pt,{}).get('reach',0) for ch in ch_list) for d in all_dates], ensure_ascii=False)
            s3a_traces_js = []
